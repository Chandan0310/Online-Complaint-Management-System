/**
 * @file Profile.jsx
 * @description Displays the logged-in user's profile information and
 *              provides a secure password-change form with regex-enforced
 *              complexity requirements.  Works for all roles (Student,
 *              Manager, Admin).
 */
...
/** Role-specific configuration for badge styling and navigation. */
const ROLE_CONFIG = {
  STUDENT: {
    label: 'Student',
    icon: 'bi-mortarboard',
...
/**
 * Profile component — profile card (left) and password-change
 * form (right).  Navigable from any dashboard top-bar.
 *
  ...
  return (
    <div>
      ...
      <div className="ocms-topbar">
        <button className="btn-outline-modern" onClick={() => navigate(config.backPath)}>
          ...
          <div className="col-md-4">
            <div className="card-modern h-100">
              <div className="card-modern-header gradient">
                <h5><i className="bi bi-person-badge"></i> My Profile</h5>
              </div>
              <div className="card-modern-body" style={{ textAlign: 'center', paddingTop: '2rem' }}>
                ...
                <span className={`badge-status ${config.badgeClass}`} style={{ marginBottom: '1.5rem', display: 'inline-flex' }}>
